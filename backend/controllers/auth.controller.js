...
const signIn = asyncErrorHandler(async (req, res) => {
  ...
  const validUser = await User.findOne({ email }, { createdAt: 0, __v: 0, updatedAt: 0 });
...
const generateToken = asyncErrorHandler(async (req, res) => {
  const cookies = req.cookies.refreshToken;
  if (!cookies) throw new CustomError("Unauthorized Access", 401);
  ...
  if (!freshUser) throw new CustomError("User not found", 404);

  const token = jwt.sign(
    { id: freshUser._id, email: freshUser.email, role: freshUser.role },
    process.env.JWT_SECRET,
    { expiresIn: "1h" }
  );

  return res.status(200).json({ accessToken: token, ...freshUser._doc });
});

const logoutUser = asyncErrorHandler((req, res) => {
...
const forgotPasswordHandler = (req, res, next) => {
  ...
  if (step === STEPS.SENDOTP) return forgotPassword(req, res, next);
  else if (step === STEPS.VERIFYOTP) return verifyOTP(req, res, next);
  else if (step === STEPS.UPDATEPASSWORD) return updatePassword(req, res, next);
...
const forgotPassword = asyncErrorHandler(async (req, res) => {
  ...
  if (!email) throw new CustomError("Please provide email address", 400);

  const user = await User.findOne({ email });
  if (!user) throw new CustomError("No user with given email present", 400);

  const otp = Math.floor(100000 + Math.random() * 900000);
  const newToken = new Token({ userId: user._id, otp });
  await newToken.save();

  res.cookie("tokenId", newToken._id, {
    httpOnly: true,
    secure: true,
    sameSite: "None",
    maxAge: 15 * 60 * 1000,
...
const verifyOTP = asyncErrorHandler(async (req, res) => {
  const { otp } = req.body;
  if (!otp) throw new CustomError("Please provide OTP", 400);

  const tokenId = req.cookies?.tokenId;
  if (!tokenId) throw new CustomError("OTP expired, consider sending it again", 400);

  const matchedToken = await Token.findById(tokenId);
  if (!matchedToken) throw new CustomError("OTP expired, consider sending it again", 400);
  ...
  res.cookie("userId", matchedToken.userId, {
    httpOnly: true,
    secure: true,
    sameSite: "None",
    maxAge: 15 * 60 * 1000,
...
const updatePassword = asyncErrorHandler(async (req, res) => {
  ...
  if (!userId) throw new CustomError("OTP expired, consider sending it again", 400);
